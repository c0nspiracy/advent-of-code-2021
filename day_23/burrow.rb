...
class Burrow
  ...
  def move(cost, type, source_index, target_index)
    new_burrow = dup
    case type
    when :r
  ...
  def valid_space_to_room_moves
    ...
    spaces.each_with_index do |amphipod, space_index|
      ...
      cost = space_to_room_cost(space_index, target_room_index)
      valid_moves << [cost, :s, space_index, target_room_index]
    end
  ...
  def valid_room_to_space_moves
    ...
    tops_of_stacks.each_with_index.each do |amphipod, room_index|
      ...
      valid_spaces.sort_by { |s| (space_above_room - s).abs }.each do |space_index|
        cost = room_to_space_cost(room_index, space_index)
        valid_moves << [cost, :r, room_index, space_index]
      end
    end
